//
//  YMCommonStrings.swift
//  YiMai
//
//  Created by why on 16/4/16.
//  Copyright © 2016年 why. All rights reserved.
//

import Foundation

public class YMCommonStrings {
    public static let CS_HOME_LABEL_TITLE = "主页"
    public static let CS_YIMAI_LABEL_TITLE = "医脉"
    public static let CS_PERSONAL_LABEL_TITLE = "个人"
    
    public static let CS_PAGE_INDEX_NAME = "PageIndex"
    
    
    public static let CS_PAGE_LOGIN_NAME = "PageLogin"
    public static let CS_PAGE_REGISTER_NAME = "PageRegister"

    public static let CS_PAGE_SELECT_CITY_NAME = "PageSelectCity"

    public static let CS_PAGE_REGISTER_PERSONAL_INFO_NAME = "PageRegisterInputInfo"
    public static let CS_PAGE_GET_MY_DOCTORS_NAME = "PageGetMyDoctors"
    public static let CS_PAGE_GET_DEFAULT_SEARCH = "PageSearchDefault"

    public static let CS_PAGE_APPOINTMENT_SELECT_TIME = "PageAppointmentSelectTime"
    public static let CS_PAGE_APPOINTMENT = "PageAppointment"
    
    public static let CS_PAGE_APPOINTMENT_PATIENT_BASIC_INFO_NAME = "PageAppointmentBasicInfo"
    public static let CS_PAGE_APPOINTMENT_PATIENT_CONDITION_NAME = "PageAppointmentPatientCondition"
    
    public static let CS_PAGE_APPOINTMENT_PROXY_PATIENT_BASIC_INFO_NAME = "PageAppointmentProxyBasicInfo"
    public static let CS_PAGE_APPOINTMENT_PROXY_PATIENT_CONDITION_NAME = "PageAppointmentProxyPatientCondition"
    public static let CS_PAGE_APPOINMENT_PROXY_NAME = "PageAppointmentProxy"

    public static let CS_API_PARAM_KEY_TOKEN = "token"
    public static let CS_API_PARAM_KEY_PHONE = "phone"
    public static let CS_API_PARAM_KEY_ID = "id"
    public static let CS_API_PARAM_KEY_CODE = "code"
    public static let CS_API_PARAM_KEY_FRIEND_ID = "friend_id"
    public static let CS_API_PARAM_KEY_REMARKS = "remarks"
    public static let CS_API_PARAM_KEY_ID_LIST = "id_list"
    public static let CS_API_PARAM_KEY_PAGE = "page"
    public static let CS_API_PARAM_KEY_HOSPITAL = "hospital"
    public static let CS_API_PARAM_KEY_NAME = "name"
}






























